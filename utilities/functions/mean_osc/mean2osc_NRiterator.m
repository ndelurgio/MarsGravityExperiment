function osc_equi_elem = mean2osc_NRiterator(mean_equi_elem, tol, mu, J2, Re)
...
osc_equi_elem = mean_equi_elem;
R = 1;
niter = 0;
while abs(R) > tol
  niter = niter+1;
  [~, osc_loop, ~] = transformationmatrix_mean2osc_equinoctial(osc_equi_elem, mu, J2, Re);
  delta = osc_equi_elem - osc_loop;
  R = norm(delta, inf);
  osc_equi_elem = osc_equi_elem + delta;
  if niter > 100
